...
module.exports = function (element) {
  ...
  switch (element[0].name) {
    ...
    default:
      ...
      // <button-secondary>
    case this.components.buttonSecondary:
      var expander = '';
      ...
      // The .button class is always there, along with any others on the <button-secondary> element
      ...
      return format('<table class="%s"><tbody><tr><td><table><tbody><tr><td>%s</td></tr></tbody></table></td>%s</tr></tbody></table>', classes.join(' '), inner, expander);
      // <button-link>
    case this.components.buttonLink:
      var expander = '';
      ...
      // The .button class is always there, along with any others on the <button-link> element
      var classes = ['button-link'];
      if (element.attr('class')) {
        classes = classes.concat(element.attr('class').split(' '));
      }
    ...
    // <banner>
    case this.components.banner:
      var classes = ['banner'];
      if (element.attr('class')) {
        classes = classes.concat(element.attr('class').split(' '));
      }

      return format('<table %s align="center" class="%s"><tbody><tr><td class="banner__inner"><center>%s</center></td></tr></tbody></table>', attrs, classes.join(' '), inner);

    // <table-container>
    case this.components.tableContainer:
      var classes = ['table-container'];
      if (element.attr('class')) {
        classes = classes.concat(element.attr('class').split(' '));
      }

      return format('<table %s align="center" class="%s"><tbody><tr><td class="table-container__inner">%s</td></tr></tbody></table>', attrs, classes.join(' '), inner);

    //<table-content>
    case this.components.tableContent:
      var classes = ['table-content'];
      if (element.attr('class')) {
        classes = classes.concat(element.attr('class').split(' '));
      }
  
      return format('<table %s align="center" class="%s"><tbody><tr><td class="table-content__inner">%s</td></tr></tbody></table>', attrs, classes.join(' '), inner);
  }
}