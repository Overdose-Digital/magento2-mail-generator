...
module.exports = function(col) {
  ...
  var colSize = $(col).attr('col') || $(col).attr('small') || Math.floor(this.columnCount / colCount);
  var noExpander =  $(col).attr('no-expander');

  //classes.push(format('small-%s', smallSize));
  //classes.push(format('', smallSize));
  classes.push(format('col-%s', colSize));
  ...
  if (colSize == this.columnCount && col.find('.row, row').length === 0 && (noExpander == undefined || noExpander == "false") ) {
